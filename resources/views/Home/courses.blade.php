...
@section('content')
    ...
    <div class="col-lg-8">
        ...
        <!-- Author -->
        <p class="lead small">
            توسط <a href="#">{{ $courses->user->name }}</a>
        </p>
...
{{--            <source src="{{ \App\Models\Episode::find(1)->download() }}" type='video/mp4'>--}}
            <p class="vjs-no-js">
        ...
        @if(auth()->check())
            @if($courses->type == 'vip')
                ...
                @endif
            @elseif($courses->type == 'cash')
                @if(! user()->checkLearning($courses))
        ...
        <table class="table table-condensed table-bordered">
            ...
            <tbody>
            @foreach($courses->episodes as $episode)
                <tr>
                    <th>{{ $episode->number }}</th>
                    <td>{{ $episode->title }}</td>
                    <td>{{ $episode->time }}</td>
                    <td>
                        <a href="../download/{{$episode->id}}"><span class="glyphicon glyphicon-download-alt" aria-hidden="true"></span></a>
                    </td>
                </tr>
            @endforeach
            </tbody> </table>
        ...
        @include('Home.layouts.comment' , ['comments' => $comments , 'subject' => $courses])
    </div>

    <!-- Blog Sidebar Widgets Column -->

    <div class="col-md-4">
        {{--@if(! auth()->user()->checkLearning($courses))--}}
        {{--<div class="well">--}}
